...
export default async function handler(req: NextApiRequest, res: NextApiResponse): Promise<void> {
  if (req.method === "GET") {
    ...
    try {
      const { data, error } = await supabase.from("record_tags").select("tags(id, name)").eq("record_id", recordId);
    ...
    try {
      const { error } = await supabase.from("record_tags").delete().eq("record_id", recordId).in("tag_id", tagIds);
