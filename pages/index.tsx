        ...
        cell: ({ row }) => {
          ...
          return (
            <div className="flex flex-col gap-2">
              {summary ? (
                <span className="flex flex-col gap-1">{isLong ? summary.slice(0, 50) + "..." : summary}</span>
              ) : null}
        ...
        cell: ({ row }) => {
          ...
          return (
            <div className="flex flex-col gap-1">
              {conclusion ? (
                <span className="flex flex-col gap-1">{isLong ? conclusion.slice(0, 50) + "..." : conclusion}</span>
              ) : null}
        ...
        cell: ({ row }) => (
          <div className="flex flex-wrap gap-2">
            {row.original.tags?.map((tag) => (
              <span key={tag.id} className="bg-indigo-100 text-indigo-800 text-xs font-medium px-2.5 py-0.5 rounded">
                {tag.name}
              </span>
            ))}
            <button
        ...
        cell: ({ row }) => (
          <div>
            ...
            <div className="flex flex-col items-start gap-0.5 mt-1 text-[11px] font-medium text-indigo-700 bg-indigo-50 px-2 py-1 rounded">
              <span title={row.original.creator_name || ""}>Creator: 👤 {row.original.creator_name || "N/A"}</span>
            </div>
          </div>
        ),
      },
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
          <div className="bg-white shadow-lg rounded-lg p-8 w-full max-w-4xl relative h-[90vh] overflow-hidden">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-2">
              <h1 className="text-2xl font-semibold text-gray-800">{editingRecord ? "Update" : "Upload New"} Record</h1>
              {user && (
      ...
      {(summaryOpen || conclusionOpen) && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
          <div className="bg-white shadow-lg rounded-lg p-8 w-[60vw] h-[80vh] relative flex flex-col scroll-none">
            ...
            <h2 className="text-2xl font-semibold text-gray-800 mb-4">{summaryOpen ? "Summary" : "Conclusion"}</h2>
