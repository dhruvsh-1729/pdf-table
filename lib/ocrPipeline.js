...
function normalizeOcrLanguages(languages, fallback = OCR_LANGUAGES) {
  ...
  const normalized = rawLanguages
    ...
    .filter(Boolean)
    .map((language) => OCR_LANGUAGE_ALIASES[language] || language)
    .filter((language) => /^[a-z]{3}$/.test(language));

  const unique = Array.from(new Set(normalized));
  if (unique.length) return unique;
  return fallback?.length ? fallback : ["eng"];
}
...
function deriveOutputFilename(record, suffix) {
  const baseLabel = record?.title_name || getMagazineName(record) || `record-${record?.id || "file"}`;
...
async function createOcrPdfBuffer(pdfBuffer, originalFilename, ocrLanguages, logger = console, outputFilename = "ocr.pdf") {
  const resolvedLanguages = normalizeOcrLanguages(ocrLanguages);
  const authToken = await authenticateIlove();
  const { server, task } = await startIloveTask(authToken, "pdfocr");
  logger.info?.(`Started iLovePDF task ${task} on ${server} (pdfocr:${resolvedLanguages.join(",")})`);
  ...
  await processIloveTask(authToken, server, {
    ...
    tool: "pdfocr",
    files: [{ server_filename: serverFilename, filename: originalFilename || serverFilename }],
    ocr_languages: resolvedLanguages,
    ignore_errors: true,
    try_pdf_repair: true,
  ...
  return { ocredBuffer, ocr_languages: resolvedLanguages };
}
...
export async function runOcrForPdfBuffer({
  pdfBuffer,
  ...
  const baseName = sanitizeFilenameSegment(
    path.basename(originalFilename || "file.pdf", path.extname(originalFilename || "")),
  );
  const resolvedOutputFilename = outputFilename || `${baseName}-ocr.pdf`;
  const { ocredBuffer, ocr_languages } = await createOcrPdfBuffer(
    pdfBuffer,
    originalFilename,
    ocrLanguages,
    logger,
  ...
  return {
    ...
    ocred_bytes: ocredBuffer.length,
    compression_events: compressionCount > 0 ? compression_events : undefined,
  };
}
...
export async function runOcrForRecord({
  ...
  const { ocredBuffer } = await createOcrPdfBuffer(
    pdfBuffer,
    originalFilename,
    OCR_LANGUAGES,
    logger,
    `record-${recordId}-ocr.pdf`,
  );
