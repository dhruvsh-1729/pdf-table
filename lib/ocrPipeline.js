...
const SUPPORTED_ILOVEPDF_OCR_LANGUAGES = ["eng", "hin", "sans", "guj"];
const SUPPORTED_ILOVEPDF_OCR_LANGUAGE_SET = new Set(SUPPORTED_ILOVEPDF_OCR_LANGUAGES);
...
function normalizeRequestedOcrLanguages(input) {
  if (!input) return [];
...
function resolveOcrLanguages(ocrLanguages) {
  const parsed = normalizeRequestedOcrLanguages(ocrLanguages);
  const resolved = parsed.length ? parsed : [DEFAULT_OCR_LANGUAGE];
  const invalid = resolved.filter((lang) => !SUPPORTED_ILOVEPDF_OCR_LANGUAGE_SET.has(lang));
  if (invalid.length) {
    throw new Error(
      `Unsupported OCR language code(s): ${invalid.join(", ")}. Supported values: ${SUPPORTED_ILOVEPDF_OCR_LANGUAGES.join(", ")}.`,
    );
  }
...
export async function runOcrForRecord({
  ...
  ensureUploadThingToken();
  const resolvedOcrLanguages = resolveOcrLanguages(ocrLanguages);
  ...
  logger.info?.(`Started iLovePDF task ${task} on ${server} (pdfocr:${resolvedOcrLanguages.join(",")})`);
  ...
  await processIloveTask(authToken, server, {
    ...
    tool: "pdfocr",
    files: [{ server_filename: serverFilename, filename: originalFilename || serverFilename }],
    ocr_languages: resolvedOcrLanguages,
    ignore_errors: true,
    try_pdf_repair: true,
