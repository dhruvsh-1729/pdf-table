...
function resolveMaxPdfBytes() {
  const raw =
    process.env.CLOUDINARY_PDF_MAX_BYTES || process.env.PDF_MAX_BYTES || process.env.MAX_PDF_BYTES || undefined;
...
export async function uploadWithCompressFallback(ocredBuffer, publicIdWithExt, originalFilename, options = {}) {
  ...
  while (true) {
    ...
    try {
      ...
      await compressWithLevel(nextLevel, "cloudinary-size");
      await ensureWithinLimit();
    }
  }
